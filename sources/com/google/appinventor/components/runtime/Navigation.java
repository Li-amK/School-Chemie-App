...
@DesignerComponent(category = ComponentCategory.MAPS, description = Navigation.TAG, iconName = "images/navigation.png", nonVisible = SyntaxForms.DEBUGGING, version = 1)
...
public class Navigation extends AndroidNonvisibleComponent implements Component {
    ...
    public void TransportationMethod(String method) {
        TransportMethod[] values = TransportMethod.values();
        for (TransportMethod t : values) {
    ...
    public void performRequest(GeoPoint start, GeoPoint end, TransportMethod method) throws IOException, JSONException {
        ...
        try {
            connection = (HttpURLConnection) new URL(this.serviceUrl + method.method() + "/geojson/").openConnection();
    ...
    private YailList getLineStringCoords(YailDictionary feature) {
        return GeoJSONUtil.swapCoordinates((YailList) feature.getObjectAtKeyPath(Arrays.asList("geometry", "coordinates")));
    }
