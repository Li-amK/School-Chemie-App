...
@DesignerComponent(category = ComponentCategory.LEGOMINDSTORMS, description = "A component that provides a low-level interface to a LEGO MINDSTORMS NXT robot, with functions to send NXT Direct Commands.", iconName = "images/legoMindstormsNxt.png", nonVisible = SyntaxForms.DEBUGGING, version = 1)
@SimpleObject
    ...
    public List<Integer> LsRead(String sensorPortLetter) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                list.add(Integer.valueOf(returnPackage[i + 4] & Ev3Constants.Opcode.TST));
            }
            return list;
        } catch (IllegalArgumentException e) {
                ...
                try {
                    ...
                    try {
                        ...
                        while (sentLength < fileSize) {
                            ...
                            sentLength += writeChunk("DownloadFile", handle.intValue(), buffer, chunkLength);
                        }
                        in.close();
                        tempFile.delete();
                    } finally {
