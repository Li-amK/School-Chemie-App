...
public class Analyzer {
    ...
    private static void updateSizeDependentWidgets(ConstraintWidget widget, int orientation, int containerLength) {
        ...
        if (startAnchor.mTarget == null || endAnchor.mTarget == null) {
            hasBias = false;
        } else {
            hasBias = true;
        }
    ...
    private static int getParentBiasOffset(ConstraintWidget widget, int orientation) {
        ...
        return (int) ((((widget.mParent.getLength(orientation) - startAnchor.getMargin()) - endAnchor.getMargin()) - widget.getLength(orientation)) * (orientation == 0 ? widget.mHorizontalBiasPercent : widget.mVerticalBiasPercent));
    }
