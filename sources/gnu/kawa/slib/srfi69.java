...
public class srfi69 extends ModuleBody {
    ...
    static Procedure appropriateHashFunctionFor(Object comparison) {
        ...
        Object x2;
        Object x3;
        boolean x4 = comparison == Scheme.isEq;
        if (x4) {
            x = hash$Mnby$Mnidentity;
        } else {
            x = x4 ? Boolean.TRUE : Boolean.FALSE;
        }
        if (x != Boolean.FALSE) {
            return (Procedure) x;
        }
        Object x5 = comparison == strings.string$Eq$Qu ? 1 : null;
        if (x5 != null) {
            x2 = string$Mnhash;
        } else {
            x2 = x5 != null ? Boolean.TRUE : Boolean.FALSE;
        }
        if (x2 != Boolean.FALSE) {
            return (Procedure) x2;
        }
        Object x6 = comparison == unicode.string$Mnci$Eq$Qu ? 1 : null;
        if (x6 != null) {
            x3 = string$Mnci$Mnhash;
        } else {
            x3 = x6 != null ? Boolean.TRUE : Boolean.FALSE;
        }
        return x3 != Boolean.FALSE ? (Procedure) x3 : hash;
    }
