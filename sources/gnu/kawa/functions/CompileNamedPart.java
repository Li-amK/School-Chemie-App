...
public class CompileNamedPart {
    ...
    public static Expression validateGetNamedPart(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        if (!(typeval instanceof ObjectType)) {
            ...
            PrimProcedure[] methods2 = ClassMethods.getMethods((ObjectType) typeval, Compilation.mangleName(mname), (char) 0, caller, language);
            if (methods2 == null || methods2.length <= 0) {
                ApplyExp aexp2 = new ApplyExp(SlotGet.staticField, args);
                aexp2.setLine(exp);
    ...
    public static Expression makeExp(Expression clas, Expression member) {
        ...
        if (clas instanceof ReferenceExp) {
            ...
            if (rexp.isUnknown()) {
                ...
                if (env.get(rsym instanceof Symbol ? (Symbol) rsym : env.getSymbol(rsym.toString()), (Object) null) == null) {
                    try {
    ...
    public static Expression validateSetNamedInstancePart(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        exp.visitArgs(visitor);
        Expression[] args = exp.getArgs();
        return visitor.visitApplyOnly(new ApplyExp(SlotSet.set$Mnfield$Ex, args[0], new QuoteExp(((SetNamedInstancePart) proc).pname), args[1]), required);
    }
}
