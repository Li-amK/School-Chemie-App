...
public class CompileMisc {
    ...
    public static Expression validateApplyValuesFilter(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        if (vproc.kind == 'R') {
            ...
            LetExp let = new LetExp(new Expression[]{new ApplyExp(AddOp.$Pl, new ApplyExp(AddOp.$Mn, new ReferenceExp(lastArg), new ReferenceExp(posIncoming)), new QuoteExp(IntNum.one()))});
            ...
            if (lexp2.min_args == 3 && lexp2.max_args == 3) {
                ...
                if (type1 == null || NodeType.anyNodeTest.compare(type1) != -3) {
                    ...
                    if (lastArg.getCanRead()) {
                        ...
                        comp.letEnter();
                        LetExp lastLet = new LetExp(new Expression[]{new ApplyExp(typeNodes.getDeclaredMethod("size", 0), new ReferenceExp(sequence))});
