...
class MPN {
    ...
    public static long udiv_qrnnd(long N, int D) {
        ...
        } else if (a1 < (((D - a1) - (a0 >>> 31)) & 4294967295L)) {
            q = N / D;
            r2 = N % D;
        } else {
            long c3 = N - (D << 31);
    ...
    public static int divmod_1(int[] quotient, int[] dividend, int len, int divisor) {
        ...
        while (i >= 0) {
            r = udiv_qrnnd(((-4294967296L) & r) | (dividend[i] & 4294967295L), divisor);
            quotient[i] = (int) r;
            i--;
        }
        return (int) (r >> 32);
    ...
    public static void divide(int[] zds, int nx, int[] y, int ny) {
        ...
        do {
            if (zds[j] == y[ny - 1]) {
                ...
                qhat = (int) udiv_qrnnd((zds[j] << 32) + (zds[j - 1] & 4294967295L), y[ny - 1]);
            }
            if (qhat != 0) {
                for (long num = (zds[j] & 4294967295L) - (submul_1(zds, j - ny, y, ny, qhat) & 4294967295L); num != 0; num = carry - 1) {
                    qhat--;
                    carry = 0;
                    for (int i = 0; i < ny; i++) {
    ...
    public static int set_str(int[] dest, byte[] str, int str_len, int base) {
        ...
        if (((base - 1) & base) != 0) {
            ...
            while (true) {
                ...
                if (size == 0) {
                    cy_limb = res_digit == 1 ? 1 : 0;
                } else {
                    cy_limb = mul_1(dest, dest, size, big_base) + add_1(dest, dest, size, res_digit == 1 ? 1 : 0);
                }
                if (cy_limb != 0) {
                    size2 = size + 1;
                    dest[size] = cy_limb;
            ...
            while (true) {
                ...
                byte b = str[i2];
                res_digit2 |= b << next_bitpos;
                next_bitpos += bits_per_indigit;
                if (next_bitpos >= 32) {
                    size2 = size + 1;
                    dest[size] = res_digit2;
                    next_bitpos -= 32;
                    res_digit2 = b >> (bits_per_indigit - next_bitpos);
                } else {
                    size2 = size;
                }
            }
    ...
    public static int gcd(int[] x, int[] y, int len) {
        ...
        int len2 = len - i;
        rshift0(x, x, i, len2, initShiftBits);
        rshift0(y, y, i, len2, initShiftBits);
        if ((x[0] & 1) != 0) {
            odd_arg = x;
            other_arg = y;
        } else {
        ...
        if (initShiftBits <= 0) {
            ...
            int sh_out = lshift(x, i, x, len2, initShiftBits);
            if (sh_out != 0) {
                x[len2 + i] = sh_out;
                len2++;
            }
