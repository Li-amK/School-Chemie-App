...
public class ExponentialFormat extends Format {
    ...
    StringBuffer format(double value, String dstr, StringBuffer sbuf, FieldPosition fpos) {
        ...
        int i3 = this.width - (sbuf.length() - oldLen);
        if (fracUnspecified && ((dot + 1 == sbuf.length() || sbuf.charAt(dot + 1) == this.exponentChar) && (this.width <= 0 || i3 > 0))) {
            i3--;
            sbuf.insert(dot + 1, '0');
        }
        if ((i3 >= 0 || this.width <= 0) && (!showExponent || exponentLen <= this.expDigits || this.expDigits <= 0 || this.overflowChar == 0)) {
            if (k <= 0 && (i3 > 0 || this.width <= 0)) {
                sbuf.insert(digStart, '0');
                i3--;
            }
