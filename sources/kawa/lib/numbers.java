...
public class numbers extends ModuleBody {
    ...
    public static boolean isInteger(Object x) {
        ...
        if (x3) {
            try {
                if (Math.IEEEremainder(((DFloNum) x).doubleValue(), 1.0d) == 0.0d) {
                    x3 = true;
                } else {
                    x3 = false;
                }
    ...
    public static boolean isInexact(Object x) {
        boolean x2 = x instanceof Number;
        if (x2) {
            return ((Arithmetic.isExact((Number) x) ? 1 : 0) + 1) & 1;
        }
        return x2;
    }
    ...
    public static Complex quantity$To$Number(Quantity q) {
        q.unit();
        return q.doubleValue() == 1.0d ? q.number() : Complex.make(q.reValue(), q.imValue());
    }
